...
// ─── Tasks Search with Output (Docker) ───────────────────────────────────────

test.describe("Tasks Search (Docker)", () => {
  // These tests spin up a real container; allow plenty of time.
  test.setTimeout(120000);

  test("finds a task by unique output content", async ({ page, request }) => {
    // Use an all-alphanumeric marker so FTS5 tokenizes it as a single token
    const marker = `UQMARK${Date.now()}`;

    const id = await createPipeline(
      request,
      uniqueName("tasks-srchout"),
      `export const pipeline = async () => {
        await runtime.run({
          name: "marker-task",
          image: "busybox",
          command: { path: "echo", args: ["${marker}"] }
        });
      };`,
      "docker://",
    );
    ...
    // Navigate to the tasks page for this run
    const tasksLink = page.getByRole("link", { name: "Tasks" }).first();
    await expect(tasksLink).toBeVisible({ timeout: 10000 });
    await tasksLink.click();
    ...
    // Wait for the task item to load
    await expect(page.locator(".task-item").first()).toBeVisible({
      timeout: 30000,
    });

    // Search for the unique marker in task output
    const searchInput = page.getByLabel("Search task output");
    await searchInput.fill(marker);
    await page.waitForTimeout(800);

    // The marker-task should appear in results
  ...
  test("task search shows empty results when no output matches", async ({
    ...
    const id = await createPipeline(
      ...
      `export const pipeline = async () => {
        await runtime.run({
          name: "noop-task",
          image: "busybox",
          command: { path: "echo", args: ["nothing-special"] }
        });
      };`,
      "docker://",
    );
    ...
    const searchInput = page.getByLabel("Search task output");
    await searchInput.fill("ZZZNOMATCHXYZ987");
    await page.waitForTimeout(800);

    // No task items should be visible when search doesn't match
    await expect(page.locator(".task-item")).toHaveCount(0, { timeout: 5000 });
  });
});
